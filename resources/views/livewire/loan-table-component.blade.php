...
<div>
    ...
    <!-- Placeholder for the rest of the view (loans table, etc.) -->
    <!-- Assume existing table rendering loans, pagination, and other UI elements -->
</div>
...
<div>
    <div class="wrapper wrapper-content animated fadeInRight overflow-y-scroll scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent hover:scrollbar-thumb-gray-500">
        <div class="row">
            <div class="col-lg-12">
                <div class="ibox">
                    <div class="ibox-content table-responsive border-0">
                        @unless (count($loans) == 0)
                            <div class="mt-4">
                                <section class="py-3 sm:py-5">
                                    <div class="px-4 mx-auto max-w-screen-2xl lg:px-12">
                                        <div class="relative overflow-hidden bg-white-700 shadow-md dark:bg-gray-800 sm:rounded-lg">
                                            <!-- Search bar and other things -->
                                            <div x-data="loanSearch()" @keydown.window.ctrl.k.prevent="focusSearch" @keydown.window.meta.k.prevent="focusSearch"
                                                class="flex flex-col items-center justify-between gap-4 px-4 py-3 bg-gray-700 lg:flex-row lg:gap-6">
                                                {{-- Title --}}
                                                <div class="flex items-center flex-1 px-2">
                                                    <h4 class="text-lg font-semibold text-white">Ongoing Loans</h4>
                                                    ...
                                                    <div class="relative">
                                                        ...
                                                        <span class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                                                            <svg class="w-5 h-5 text-gray-400 dark:text-gray-300" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35M10.5 17a6.5 6.5 0 1 1 0-13 6.5 6.5 0 0 1 0 13z" />
                                                            </svg>
                                                        </span>
                                                        <input type="text" id="search-loans" wire:model.live="search"
                                                            class="w-full py-2.5 ps-10 pe-24 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-gray-500 focus:border-gray-500 dark:bg-gray-800 dark:border-gray-600 dark:text-white dark:placeholder-gray-400 dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                            ...
                                            <div class="overflow-x-scroll scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent hover:scrollbar-thumb-gray-500">
                                                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                                    <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                                                        <tr>
                                                            <th scope="col" class="px-4 py-3">Name</th>
                                                            <th class="px-3 py-2 cursor-pointer" wire:click="sortBy('loan_required_amount')">
                                                                Amount Loaned
                                                                @if ($sortField === 'loan_required_amount')
                                                                    @if ($sortDirection === 'asc') <span>↑</span> @else <span>↓</span> @endif
                                                                @endif
                                                            </th>
                                                            <th class="px-3 py-2 cursor-pointer" wire:click="sortBy('amount_paid')">
                                                                Amount Paid
                                                                @if ($sortField === 'amount_paid')
                                                                    @if ($sortDirection === 'asc') <span>↑</span> @else <span>↓</span> @endif
                                                                @endif
                                                            </th>
                                                            <th class="px-3 py-2 cursor-pointer" wire:click="sortBy('amount_remaining')">
                                                                Amount Remaining
                                                                @if ($sortField === 'amount_remaining')
                                                                    @if ($sortDirection === 'asc') <span>↑</span> @else <span>↓</span> @endif
                                                                @endif
                                                            </th>
                                                            <th class="px-3 py-2 cursor-pointer" wire:click="sortBy('days_remaining')">
                                                                Days Remaining
                                                                @if ($sortField === 'days_remaining')
                                                                    @if ($sortDirection === 'asc') <span>↑</span> @else <span>↓</span> @endif
                                                                @endif
                                                    ...
                                                    <tbody class="text-gray-600 dark:text-gray-200">
                                                        @forelse ($loans as $loan)
                                                            <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 accordion-toggle cursor-pointer" data-target="#collapse-{{ $loan->id }}">
                                                                <td class="px-2 py-3">
                                                                ...
                                                                <td class="px-2 py-3 text-sm">{{ number_format($loan->loan_required_amount) }} Tsh</td>
                                                                <td class="px-2 py-3 text-sm">{{ number_format($loan->payments->sum('paid_amount')) }} Tsh</td>
                                                                <td class="px-2 py-3 text-sm">{{ number_format($loan->loan_required_amount - $loan->payments->sum('paid_amount')) }} Tsh</td>
                                                                <td class="w-auto px-6 py-3 flex items-center text-sm">
                                                                    <div class="flex items-center">
                                                                        @php
                                                                            $now = Carbon::now();
                                                                            ...
                                                                            $minutes = $interval->i;
                                                                            if ($minutes >= 30) { $hours++; }
                                                                            if ($hours >= 24) { $hours -= 24; $remainingDays++; if ($remainingDays >= 7) { $weeks += floor($remainingDays / 7); $remainingDays %= 7; } }
                                                                            $parts = [];
                                                                            if ($totalMonths > 0) { $parts[] = $totalMonths . ' m' . ($totalMonths !== 1 ? 's' : ''); }
                                                                            if ($weeks > 0) { $parts[] = $weeks . ' w' . ($weeks !== 1 ? 's' : ''); }
                                                                            if ($remainingDays > 0) { $parts[] = $remainingDays . ' d' . ($remainingDays !== 1 ? 's' : ''); }
                                                                            if ($hours > 0 || empty($parts)) { $parts[] = $hours . ' h' . ($hours !== 1 ? 's' : ''); }
                                                                            if (empty($parts)) { $parts[] = 'less than an hour'; }
                                                                            echo match (count($parts)) {
                                                                                1 => $parts[0],
                                                                                2 => implode(' and ', $parts),
                                                                ...
                                                                <td>
                                                                    <div class="text-sm">
                                                                        @php
                                                                            $time_left = $interval;
                                                                            $paid_amount = $loan->payments->sum('paid_amount');
                                                                            $loan_amount = $loan->loan_required_amount;
                                                                            if ($paid_amount >= $loan_amount) {
                                                                                $status = 'completed';
                                                                                $color_class = 'text-green-700 bg-green-100 dark:bg-green-700 dark:text-green-100';
                                                                            } elseif ($time_left->invert == 0 && $paid_amount < $loan_amount) {
                                                                                $status = 'ongoing';
                                                                                $color_class = 'text-yellow-700 bg-yellow-100 dark:bg-yellow-700 dark:text-yellow-100';
                                                                            } elseif ($time_left->invert == 1 && $paid_amount < $loan_amount) {
                                                                                $status = 'overdue';
                                                                                $color_class = 'text-red-700 bg-red-100 dark:bg-red-700 dark:text-red-100';
                                                                            } else {
                                                                                $status = 'unknown';
                                                                                $color_class = 'text-gray-700 bg-gray-100 dark:bg-gray-700 dark:text-gray-100';
                                                                            }
                                                                        @endphp
                                                                        <span class="px-2 py-1 font-semibold leading-tight {{ $color_class }} rounded-full">
                                                                            {{ ucfirst($status) }}
                                                                        </span>
                                                                    </div>
                                                                </td>
                                                                <td class="px-4 py-3">
                                                                    <svg class="accordion-chevron w-5 h-5 transform transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                                                                    </svg>
                                                                </td>
                                                            </tr>
                                                            <tr id="collapse-{{ $loan->id }}" class="hidden accordion-content transition duration-300 ease-in-out">
                                                                <td colspan="7" class="p-4 bg-gray-50 dark:bg-gray-800">
                                                                    <div class="flex flex-wrap gap-4">
                                                                        <div class="w-full sm:w-full md:w-6/12 lg:w-3/12">
                                                                            <div class="bg-white dark:bg-gray-700 text-[#2E4057] dark:text-white rounded-lg p-4 shadow-sm">
                                                                                <h5 class="mb-2 font-bold text-info dark:text-blue-300">Payment Progress</h5>
                                                                                @php
                                                                        ...
                                                                        <div class="w-full sm:w-full md:w-6/12 lg:w-4/12">
                                                                            <div class="bg-white dark:bg-gray-700 text-[#2E4057] dark:text-white rounded-lg p-4 shadow-sm">
                                                                                <h5 class="mb-2 font-bold text-info dark:text-blue-300">Loan Details</h5>
                                                                                <div class="flex items-center justify-between">
                                                                                    <div>
                                                                                        <span class="text-xs text-gray-500 dark:text-gray-400">Time to next Payment</span>
                                                                                            ...
                                                                                            @else
                                                                                                @if ($loan->status === 'approved')
                                                                                                    ...
                                                                                                    @elseif($today > $loan->loan_end_date)
                                                                                                        Loan Period Ended
                                                                                                    @else
                                                                                                        Payment Schedule Exceeds End Date
                                                                                                    @endif
                                                                                                @else
                                                                                                    No Payments (Not Approved)
                                                                                                @endif
                                                                                            @endif
                                                                                        </span>
                                                                                    </div>
                                                                                    <div class="flex gap-2">
                                                                                        <x-button.outline color="gray" icon="icons.bell"
                                                                                            onclick="confirmSendReminder({{ $loan->id }}, '{{ $loan->user->first_name }}')">
                                                                                            Send Reminder
                                                                                        </x-button.outline>
                                                                                        <x-button.outline color="green" icon="icons.plus" wire:click="openPaymentModal({{ $loan->id }})">
                                                                                            Add Payment
                                                                                        </x-button.outline>
                                                                                    </div>
                                                                                </div>
                                                                        ...
                                                                        <div class="w-full sm:w-full md:w-6/12 lg:w-4/12">
                                                                            <div class="bg-white dark:bg-gray-700 text-[#2E4057] dark:text-white rounded-lg p-1 shadow-sm">
                                                                                <div id="map-{{ $loan->id }}" class="mini-map rounded-2xl w-full h-[200px] p-1 shadow-sm cursor-pointer transition-all duration-500 ease-in-out"
                                                                                    onclick="toggleMapSize('map-{{ $loan->id }}')"></div>
                                                                            </div>
                                                                        </div>
                                                                        <div id="backdrop-{{ $loan->id }}" class="backdrop" onclick="toggleMapSize('map-{{ $loan->id }}')"></div>
                                                                    </div>
                                                                </td>
                                                            </tr>
                                                        @empty
                                                            <tr>
                                                                <td colspan="100%" class="text-center py-4 text-gray-500">No loans found for "{{ $search }}"</td>
                                                            </tr>
                                                        @endforelse
                                                    </tbody>
                                                </table>
                                            </div>
                                            <div class="my-4">{{ $loans->onEachSide(1)->links('vendor.pagination.tailwind') }}</div>
                                        </div>
                                    </div>
                                </section>
                            </div>
        ...
        <!-- Payment Modal -->
        @if ($showPaymentModal)
            <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50" wire:loading.class="opacity-50 pointer-events-none">
                <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
                    <h2 class="text-lg font-semibold mb-4 text-gray-800">Add Payment</h2>
                    <form wire:submit.prevent="addPayment">
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" wire:model.live="paymentAmount" id="paymentAmount"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" min="1000" step="1">
        ...
        <script>
            ...
            function showFallbackNotification(message, type = 'info') {
                ...
                div.className = `fixed top-4 right-4 z-50 p-4 rounded shadow-lg text-white ${type === 'success' ? 'bg-green-600' : 'bg-red-600'}`;
        ...
        <style>
            ...
            .mini-map { width: 100%; height: 100px; transition: all 0.3s ease; }
            .expanded {
                position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 70vw; height: 70vh !important;
                z-index: 1000; background-color: white; border-radius: 1rem; padding: 0;
            }
            .backdrop {
                position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0, 0, 0, 0.5); z-index: 999; display: none;
            }
            .expanded+.backdrop { display: block; }
            [id^="map-"] { width: 100%; height: 100px; transition: all 0.3s ease; }
            [id^="map-"].expanded { width: 80vw !important; height: 70vh !important; }
        </style>
        ...
        <script>
            ...
            function toggleMapSize(mapId) {
                ...
                backdrop.style.display = isExpanding ? 'block' : 'none';
                setTimeout(() => {
                    if (mapContainer._mapbox) { mapContainer._mapbox.resize(); }
                }, 350);
            }
            ...
            function confirmSendReminder(loanId, name) {
                if (confirm(`Tuma kumbukumbu ya malipo kwa ${name}?`)) {
                    Livewire.dispatch('confirm-send-reminder', { loanId });
                }
            }
        </script>
    </div>
